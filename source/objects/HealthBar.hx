...
class HealthBar extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float) {
		...
		var targetValue:Null<Float> = FlxMath.remapToRange(FlxMath.bound(valueFunction(), bounds.min, bounds.max), bounds.min, bounds.max, 0, 100);
		// Lerp the current value towards the target value
		if (targetValue != null) {
			percent = FlxMath.lerp(percent, targetValue, 0.5);
		}
