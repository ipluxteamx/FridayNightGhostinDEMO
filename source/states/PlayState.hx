...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['BADDEST!', 0.2], //From 0% to 19%
		['SHIT', 0.4], //From 20% to 39%
		['BAD', 0.5], //From 40% to 49%
		['EHISH', 0.6], //From 50% to 59%
		['MEH', 0.69], //From 60% to 68%
		['NICE!', 0.7], //69%
		['GOOD', 0.8], //From 70% to 79%
		['GREAT', 0.9], //From 80% to 89%
		['SICK!', 1], //From 90% to 99%
		['FULL COMBO!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		healthBar = new HealthBar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.91 : 0.12), 'healthBar', function() return health, 0, 2);
		...
		// infoBar = new FlxBar(0, (healthBar.y - healthBar.height) - 10, TOP_TO_BOTTOM, FlxG.width, (healthBar.height - scoreTxt.y) + 10);
		// infoBar.scrollFactor.set();
		// infoBar.createFilledBar(FlxColor.BLACK, FlxColor.BLACK);
		// infoBar.visible = !ClientPrefs.data.hideHud;

		scoreTxt = new FlxText(0, healthBar.y + 28, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1;
		scoreTxt.visible = !ClientPrefs.data.hideHud;

		botplayTxt = new FlxText(400, 0, 0, "BOTPLAY", 32);
		...
		// flxtext(x, y, width, text, size)
		timeTxt = new FlxText(0, 0, 0, "", 20);
		...
		var timeIcon:FlxSprite = new FlxSprite(timeTxt.x - 30, timeTxt.y + 3).loadGraphic(Paths.image('timeicon'), true, 16, 16);
		timeIcon.scrollFactor.set();
		timeIcon.visible = updateTime = showTime;
		if(ClientPrefs.data.downScroll) timeIcon.y = FlxG.height - 44;
		...
		var infoBar:FlxSprite = new FlxSprite(0, (healthBar.y - healthBar.height) - 15).makeGraphic(FlxG.width, Std.int((scoreTxt.y + 16) + 15), FlxColor.BLACK);
		infoBar.alpha = 0.5;
		add(infoBar);
	...
	override public function update(elapsed:Float)
	{
		...
		if(!inCutscene && !paused) {
			FlxG.camera.followLerp = FlxMath.bound(elapsed * 4.5 * cameraSpeed * playbackRate / (FlxG.updateFramerate / 60), 0, 1);
		...
		var mult:Float = FlxMath.lerp(0.75, iconP1.scale.x, FlxMath.bound(1 - (elapsed * 18 * playbackRate), 0, 1));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(0.75, iconP2.scale.x, FlxMath.bound(1 - (elapsed * 18 * playbackRate), 0, 1));
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 0.825));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 0.825));
		}
	...
	public function endSong()
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				{
					Mods.loadTopMod();
					FlxG.sound.playMusic(Paths.music(states.TitleState.menuMusic), states.TitleState.soundVolume);
					#if desktop DiscordClient.resetClientID(); #end
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			*/
			if(SONG.notes[Math.floor(curStep / 16)].mustHitSection == false && !note.isSustainNote)
			{
				if (!dad.stunned)
				{
					// camera move on note hit
					switch(Std.int(Math.abs(note.noteData)))
					{
						case 0:
							camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
							camFollow.x += (dad.cameraPosition[0] + opponentCameraOffset[0]) - cameraMovingOffset;
							camFollow.y += (dad.cameraPosition[1] + opponentCameraOffset[1]);
						case 1:
							camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
							camFollow.x += (dad.cameraPosition[0] + opponentCameraOffset[0]);
							camFollow.y += (dad.cameraPosition[1] + opponentCameraOffset[1]) + cameraMovingOffset;
						case 2:
							camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
							camFollow.x += (dad.cameraPosition[0] + opponentCameraOffset[0]);
							camFollow.y += (dad.cameraPosition[1] + opponentCameraOffset[1]) - cameraMovingOffset;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
				...
				if(SONG.notes[Math.floor(curStep / 16)].mustHitSection == true && !note.isSustainNote) {
					if (!boyfriend.stunned){
						switch(Std.int(Math.abs(note.noteData))){				 
							case 0:
								camFollow.setPosition(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);
								camFollow.x += (boyfriend.cameraPosition[0] - boyfriendCameraOffset[0]) - cameraMovingOffset;
								camFollow.y += (boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]);	
							case 1:
								camFollow.setPosition(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);
								camFollow.x += (boyfriend.cameraPosition[0] - boyfriendCameraOffset[0]);
								camFollow.y += (boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]) + cameraMovingOffset;			
							case 2:
								camFollow.setPosition(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);
								camFollow.x += (boyfriend.cameraPosition[0] - boyfriendCameraOffset[0]);
								camFollow.y += (boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]) - cameraMovingOffset;
			...
			// TODO: do the cam follow bullshit
